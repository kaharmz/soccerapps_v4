...
class DetailLeagueActivity : AppCompatActivity(), DetailLeagueView {
    ...
    private var nameLeagueDetail: String? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_detail_league)
        idLeagueDetail = intent.getStringExtra("idLeague")
        nameLeagueDetail = intent.getStringExtra("nameLeague")
        supportActionBar?.title = nameLeagueDetail
        ...
        return when (item.itemId) {
            R.id.detail_list -> {
                startActivity<ListEventActivity>(
                    ...
                    "nameLeague" to leagues.nameLeague
                )
                true
            }
